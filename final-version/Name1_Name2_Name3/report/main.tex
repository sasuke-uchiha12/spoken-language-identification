...
\midrule
Assignment baseline (MMS-300m, provided) & 0.3196 & -- & -- \\
mHuBERT untuned (\texttt{20260301\_094826}) & 0.4676 & 0.4142 & 2.2854 \\
mHuBERT tuned (\texttt{20260301\_150502}) & \textbf{0.5270} & \textbf{0.4748} & \textbf{2.1629} \\
\bottomrule
\end{tabular}
...
\textbf{Data-centric mitigation (train-only):}
\begin{itemize}
...
\begin{itemize}
    \item Shared encoder + language head + adversarial speaker head.
    \item Gradient reversal layer to suppress speaker-identifiable directions.
    ...
    \item Final DANN run intentionally used augmentation off to isolate model-based effect.
\end{itemize}
...
Key outcomes:
\begin{itemize}
    \item DANN outperforms tuned no-mitigation by +0.0306 accuracy and +0.0678 macro-F1.
    \item DANN outperforms best data-centric run (Mitigation 1) by +0.0188 accuracy and +0.0558 macro-F1.
    \item Data-centric methods are sensitive to augmentation strength: Mitigation 1 worked best; stronger mixed augmentation (Mitigation 4) underperformed.
\end{itemize}
...
These are plausible due to linguistic/acoustic proximity and insufficient class-separating cues in short utterances. DANN reduces several high-volume confusions, but not all.

\begin{table}[t]
\centering
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{0.49\columnwidth}rrr@{}}
\toprule
Pair (true$\rightarrow$pred) & Untuned & DANN & $\Delta$ \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Top confusion shifts from untuned reference to DANN.}
\label{tab:confusion_shift}
\end{table}
