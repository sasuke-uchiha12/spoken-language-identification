...
\begin{abstract}
This report covers all three project tasks for Indic spoken language identification on \texttt{badrex/nnti-dataset-full}. For Task 1, we reproduced baseline pipelines and compared pretrained backbones. For Task 2, we addressed speaker shortcut bias with data-centric augmentation and model-based adversarial training (DANN). For Task 3, we analyzed bias sources, confusion patterns, and representation behavior with t-SNE. Across complete runs, the best model in our branch is DANN over mHuBERT, improving validation accuracy from 0.4676 (untuned mHuBERT baseline) to 0.5576 and macro-F1 from 0.4142 to 0.5426, while preserving speaker-disjoint validation integrity.
\end{abstract}

\section{Introduction}
Spoken language identification (SLID) in low-resource, multi-speaker settings is vulnerable to shortcut learning: models may use speaker identity cues instead of language-specific cues. Our project was organized into three tasks: (1) reproduce and improve baseline performance, (2) mitigate speaker bias, and (3) provide analysis supported by quantitative diagnostics and visualization.

Our implementation is based on PyTorch \citep{paszke2019pytorch} and Hugging Face Transformers/Datasets \citep{wolf2020transformers}. We evaluated pretrained speech encoders including MMS \citep{pratap2023mms}, wav2vec 2.0 \citep{baevski2020wav2vec2}, and mHuBERT (HuBERT family \citep{hsu2021hubert}).

\section{Data and Experimental Protocol}
Dataset: \texttt{badrex/nnti-dataset-full} \citep{badr2026dataset} with predefined \texttt{train}/\texttt{validation} splits.
...
All model selection is from validation metrics with best-checkpoint loading enabled. We report accuracy, macro-F1, eval loss, per-language accuracy, confusion matrices, and speaker diagnostics.
...
The strongest Task 1 path in our environment is mHuBERT with tuning, improving over untuned mHuBERT by +0.0594 accuracy and +0.0607 macro-F1. This tuned run is used as the no-mitigation reference for Task 2.

\section{Task 2: Speaker-Bias Mitigation}
\subsection{Problem and Approaches}
Bias risk: with limited speaker diversity per language in training, the model can minimize loss using speaker-correlated signals.
...
\begin{itemize}
    ...
    \item Intermediate ablations (Mitigation 2, 3A, 4) to study tradeoffs.
\end{itemize}
...
Compared to untuned baseline mHuBERT, DANN improves by +0.0900 accuracy and +0.1284 macro-F1. Compared to Mitigation 1, DANN still improves by +0.0188 accuracy and +0.0558 macro-F1.

Speaker-oriented evidence:
\begin{itemize}
    \item Unseen-speaker validation accuracy: 0.4676 (untuned) $\rightarrow$ 0.5576 (DANN).
    \item Overlap speaker count remains 0 (no leakage warning).
    \item Key bias-sensitive confusions reduced, e.g., \texttt{sindhi$\rightarrow$punjabi}: 79 $\rightarrow$ 14.
\end{itemize}

This indicates successful bias mitigation, but not complete removal of speaker information.
...
Interpretation: Mitigation 1 gave a stable data-centric gain; Mitigation 2 gave weaker gains; Mitigation 3A regressed; DANN gave the strongest final improvement, with higher training cost but better accuracy/F1 tradeoff.

\section{Task 3: Analysis}
\subsection{Source of Bias and Learning Difficulty}
The data regime (few train speakers per language, fully unseen validation speakers) creates a strong shortcut risk. Early optimization can latch onto speaker timbre/prosody, while weak classes remain under-separated. This is reflected by near-zero classes in untuned baseline (e.g., maithili, konkani) and concentrated confusion clusters.
...
\end{itemize}
Reasons are plausible linguistic/acoustic proximity and class difficulty imbalance. DANN reduces several dominant pairs but does not eliminate all structural confusion.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.47\textwidth]{../indic-SLID-mac/SLID_utter-project-mHuBERT-147_1e-05_20260301_094826/diagnostics/confusion_matrix_validation.png}
    \hfill
    \includegraphics[width=0.47\textwidth]{../indic-SLID-mac/SLID_utter-project-mHuBERT-147_1e-05_20260305_022125/diagnostics/confusion_matrix_validation.png}
    \caption{Validation confusion matrices: untuned baseline (left) vs DANN improved (right).}
    \label{fig:cm-bu-dann}
\end{figure*}

\subsection{Representation Analysis (t-SNE)}
We used t-SNE \citep{maaten2008tsne} on last-layer validation embeddings for the improved run. Baseline/tuned/M1 t-SNE export was not reproducible from current local artifacts because their run folders do not contain full model/preprocessor checkpoint files.
...
\end{itemize}
Interpretation: speaker identity is reduced but still partially encoded.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.48\linewidth]{../indic-SLID-mac/SLID_utter-project-mHuBERT-147_1e-05_20260305_022125/diagnostics/tsne_validation_by_language.png}
    \hfill
    \includegraphics[width=0.48\linewidth]{../indic-SLID-mac/SLID_utter-project-mHuBERT-147_1e-05_20260305_022125/diagnostics/tsne_validation_by_speaker_topk.png}
...
We added extra quantitative analyses to avoid single-metric conclusions:
\begin{itemize}
...
We report the exact run IDs and artifacts used in analysis (\texttt{reports/artifacts\_used.md}). Core reproducibility settings:
\begin{itemize}
    \item Seed: 42.
...
\end{itemize}
These controls reduce accidental variance and support fair mitigation comparison.

\FloatBarrier

\section{Conclusion}
Across all three tasks, mHuBERT provided the strongest backbone in our environment. Data-centric mitigation improved balanced performance, and DANN gave the best final model. The final DANN run achieved 0.5576 validation accuracy and 0.5426 macro-F1 with clean speaker-disjoint diagnostics. Remaining challenges are concentrated in a small set of linguistically close language pairs.

\section*{Limitations}
First, the strongest t-SNE comparison currently uses improved-run embeddings, while baseline/tuned/M1 t-SNE export is limited by missing checkpoint artifacts in those run folders. Second, some persistent confusions (e.g., konkani-marathi) remain unresolved and may require additional data balancing or task-specific objectives. Third, compute constraints limited the number of long-run and combined-mitigation experiments.
